define('loopVisualizer', ['three', 'improvedNoise'], function(THREE, ImprovedNoise) {
	'use strict';
	...
	function init(analyser, scene) {
		...
		////////INIT audio in
		freqByteData = new Uint8Array(analyserRef.frequencyBinCount);
		timeByteData = new Uint8Array(analyserRef.frequencyBinCount);
	...
	function update(noisePos) {
		analyserRef.getByteFrequencyData(freqByteData);
		analyserRef.getByteTimeDomainData(timeByteData);
		...
		var n = Math.abs(ImprovedNoise.noise(noisePos, 0, 0));
		colors.push(n);
		colors.shift(1);
